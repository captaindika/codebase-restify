...
class Address {
  ...
  async getAddresses() {
    ...
    if (address.err) {
      logger.error(ctx, 'error', 'Can\t find addresses', address.err);
  ...
  async getAdress(payload) {
    ...
    if (address.err) {
      logger.error(ctx, 'error', 'Can\'t find address', address.err);
      return wrapper.error(new NotFoundError('Can\'t find address'));
    }
    const { data } = address;
